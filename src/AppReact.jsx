...
// --- "Banco de Dados" Interno com os Textos Completos ---
...
// --- URL da Imagem do Logo Cavalcante Reis Advogados ---
...
const Header = ({ theme, toggleTheme }) => (
  <header
    className={`p-4 flex justify-between items-center ${
      theme === "light" ? "bg-white/80 backdrop-blur-sm" : "bg-gray-900/80 backdrop-blur-sm"
    } border-b ${theme === "light" ? "border-gray-200" : "border-gray-700"} sticky top-0 z-20 h-16`}>
    <div className="flex items-center gap-2">
      <FileText size={28} className={theme === "light" ? "text-blue-900" : "text-blue-300"} />
      <h1
        className={`text-2xl font-bold ${
          theme === "light" ? "text-" + colors.light.headline : "text-" + colors.dark.headline
        }`}>
    ...
    <button
      onClick={toggleTheme}
      style={{
        backgroundColor: theme === "light" ? colors.light.paragraph : colors.dark.main,
        color: theme === "light" ? colors.light.background : colors.dark.background,
      }}
      className="p-2 rounded-full transition-all duration-300 hover:opacity-80 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-pink-500"
  ...
  return (
    <aside
      ...
      <div className="flex items-center gap-3 mb-6">
        <Settings size={24} style={{ color: themeColors.headline }} />
        <h2 className="text-2xl font-bold" style={{ color: themeColors.headline }}>
          Personalizar Proposta
        </h2>
      </div>

      <div className="mb-6">
        <label htmlFor="municipio" className="block text-sm font-medium mb-1" style={{ color: themeColors.paragraph }}>
      ...
      <div className="mb-6">
        <label htmlFor="data" className="block text-sm font-medium mb-1" style={{ color: themeColors.paragraph }}>
      ...
      <hr className="my-6" style={{ borderColor: themeColors.sidebarBorder }} />

      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-semibold" style={{ color: themeColors.headline }}>
        ...
        <div className="flex gap-2">
          <button
            ...
            style={{ backgroundColor: colors.light.main, color: colors.light.headline }}>
            Todos
          </button>
          <button
            onClick={() => toggleAllServices(false)}
            className="text-xs p-1 px-2 rounded hover:opacity-80"
            style={{ backgroundColor: colors.light.sidebarBorder, color: colors.light.paragraph }}>
            Nenhum
          </button>
        </div>
      </div>
        ...
        {Object.entries(allServices).map(([key, name]) => (
          <label key={key} className="flex items-center gap-3 cursor-pointer">
            <input
              ...
              className="h-5 w-5 rounded border-gray-300 focus:ring-pink-500"
              style={{ accentColor: colors.light.button }}
            />
            <span style={{ color: themeColors.paragraph }} className="select-none">
              {name}
            </span>
          </label>
        ))}
...
const CopyButton = ({ theme, textToCopy }) => {
  ...
  const handleCopy = () => {
    try {
      ...
      if (success) {
        ...
        console.warn("Falha ao copiar como HTML, tentando como texto...");
        try {
          const structuredText = convertHtmlToStructuredText(htmlToCopy);
          ...
          setCopied(true);
          setCopyHint("Texto (sem formatação) Copiado!");
          setTimeout(() => {
            setCopied(false);
            setCopyHint("Copiar Texto da Proposta");
          }, 3000);
        } catch (fallbackErr) {
          console.error("Falha ao copiar como texto simples também:", fallbackErr);
          setCopyHint("Erro ao copiar!");
          console.error("Não foi possível copiar o conteúdo automaticamente.");
          setTimeout(() => {
            setCopyHint("Copiar Texto da Proposta");
          }, 3000);
        }
      ...
      setCopyHint("Erro!");
      console.error("Ocorreu um erro inesperado ao tentar copiar.");
      setTimeout(() => {
        setCopyHint("Copiar Texto da Proposta");
      }, 3000);
    }
  ...
  const convertHtmlToStructuredText = (html) => {
    const tempDiv = document.createElement("div");
    tempDiv.innerHTML = html;
    let text = "";
    const logoImg = tempDiv.querySelector(".proposal-header img");
    if (logoImg) text += `[LOGO: ${logoImg.alt}]\n\n`;

    tempDiv.querySelectorAll("p, h1, h2, h3, ul, ol, table").forEach((block) => {
      if (block.closest && block.closest(".proposal-header")) return;

      if (block.tagName === "P") {
        let pText = "";
        block.childNodes.forEach((node) => {
          if (node.nodeType === Node.TEXT_NODE) pText += node.textContent;
          else if (node.nodeName === "STRONG") pText += `*${node.textContent}*`;
          else pText += node.textContent;
        });
        text += pText.trim() + "\n\n";
      } else if (block.tagName === "H1" || block.tagName === "H2" || block.tagName === "H3")
        text += block.textContent.trim().toUpperCase() + "\n\n";
      else if (block.tagName === "UL" || block.tagName === "OL") {
        block.querySelectorAll("li").forEach((item) => {
          let itemTextContent = "";
          item.childNodes.forEach((node) => {
            if (node.nodeType === Node.TEXT_NODE) itemTextContent += node.textContent;
            else if (node.nodeName === "STRONG") itemTextContent += `*${node.textContent}*`;
            else itemTextContent += node.textContent;
          });
          const trimmedItemText = itemTextContent.trim();
          text += /^\d+\.\d+(\.\d+)?/.test(trimmedItemText) ? trimmedItemText + "\n" : `- ${trimmedItemText}\n`;
        });
        text += "\n";
      } else if (block.tagName === "TABLE") {
        block.querySelectorAll("tr").forEach((row) => {
          row.querySelectorAll("th, td").forEach((cell) => (text += cell.textContent.trim() + "\t"));
          text += "\n";
        });
        text += "\n";
      } else text += block.textContent.trim() + "\n\n";
    });
...
// --- Componente: O Documento de Proposta (Preview) ---
const ProposalDocument = ({ theme, options, services }) => {
  ...
  const generatedHtml = useMemo(() => {
    ...
    const renderTableObjetosRow = (serviceKey, tese, cabimento) => {
      if (!services[serviceKey]) return "";
      return `
        <tr class="border-b" style="border-color: ${themeColors.tableBorder};">
          <td class="p-2 align-top border-r" style="border-color: ${themeColors.tableBorder};">${tese}</td>
    ...
    const onErrorHandlerString = `this.onerror=null; this.alt='Falha ao carregar logo'; this.src='https://placehold.co/300x80/${themeColors.docBg.substring(
      1
    )}/${themeColors.docText.substring(1)}?text=Logo+Indisponivel';`;
    ...
    return `
      ...
      <div class="mb-10">
        <h2 class="text-2xl font-bold border-b-2 pb-2 mb-4" style="border-color: ${
      ...
      <div class="mb-10 proposal-section">
        ...
        <table class="w-full border-collapse border mb-4" style="border-color: ${themeColors.tableBorder};">
          <thead>
            <tr class="text-left font-bold border-b-2" style="background-color: ${
              themeColors.tableHeaderBg
            }; border-color: ${themeColors.tableBorder};">
              <th class="p-2 border-r" style="border-color: ${themeColors.tableBorder};">TESE</th>
              <th class="p-2">CABIMENTO / PERSPECTIVA</th>
            </tr>
          </thead>
          <tbody>
      ...
      <div class="mb-10 proposal-section">
        ...
        <div class="mt-10 pt-10 border-t text-center space-y-1" style="border-color: ${themeColors.tableBorder};">
    ...
    <>
      <style>
        {`
          ...
          #proposal-content h2 { margin-top: 2rem; margin-bottom: 0.75rem; padding-bottom: 0.5rem; border-bottom-width: 2px; }
          ...
          #proposal-content table { margin-top: 1rem; margin-bottom: 1rem; border-collapse: collapse; width: 100%; }
          #proposal-content th, #proposal-content td { padding: 0.5rem; text-align: left; vertical-align: top; }
          #proposal-content thead tr { border-bottom-width: 2px; }
          #proposal-content tbody tr { border-bottom-width: 1px; }
          #proposal-content td:first-child, #proposal-content th:first-child { border-right-width: 1px; }
        `}
      </style>
        ...
        <div className="app-container">
          <main>
            <div
              ...
              style={{ borderColor: themeColors.sidebarBorder }}>
              <ControlsSidebar
                theme={theme}
                options={options}
            ...
            <div className="main-content-area" style={{ backgroundColor: themeColors.background }}>
